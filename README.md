...
The repository also includes a deterministic TermiX A2A host (`npm run runtime:termix`). It accepts only an agent-scoped runtime token, refuses to start when wallet signing material is present, stops before the token expires, deduplicates overlapping inbox polls, answers service inquiries from fixed provider contracts, and escalates delivery, challenge, and operator-case threads instead of auto-acting. One dedicated Lending Rescue identity uses a separate root-only signer service to rotate only its scoped runtime token; the poller never receives the owner key. The first automatic rotation has not yet been observed, so public presence remains verified live state rather than a durable uptime claim.
